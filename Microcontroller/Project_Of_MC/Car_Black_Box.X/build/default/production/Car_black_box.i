...
char login(unsigned char key,unsigned char reset_flag)
{
    static unsigned char npassword[4],spassword[4];
    static char attempts_rem = 3,i;
    ...
    if(i == 4)
    {
        ...
        else
        {
            ...
            if(attempts_rem == 0)
            {
               ...
               while(sec)
               {
                   clcd_putch((sec/100) + '0',(0xC0 + 10));
                   clcd_putch(((sec/10)%10) + '0',(0xC0 + 11));
            ...
            else
            {
                ...
                clcd_print("Attempts Rem ",(0xC0 + 3));
...
void view_log(unsigned char key,unsigned char reset_flag)
{
    ...
    else
    {
        ...
        if(key == 0x37 && ee_pos < (event_count - 1) )
        {
            ee_pos++;

        }
        else if(key == 0x2F && ee_pos > 0)
        {
            ee_pos--;
        }
        for(int i = 0; i < 10;i++)
        ...
        clcd_putch(ee_pos%10 + '0',(0xC0 + 0));

        clcd_putch(v_log[0],(0xC0 + 2));
        clcd_putch(v_log[1],(0xC0 + 3));
        clcd_putch(':',(0xC0 + 4));

        clcd_putch(v_log[2],(0xC0 + 5));
        clcd_putch(v_log[3],(0xC0 + 6));
        clcd_putch(':',(0xC0 + 7));
        clcd_putch(v_log[4],(0xC0 + 8));
        clcd_putch(v_log[5],(0xC0 + 9));

        clcd_putch(v_log[6],(0xC0 + 11));
        clcd_putch(v_log[7],(0xC0 + 12));

        clcd_putch(v_log[8],(0xC0 + 14));
        clcd_putch(v_log[9],(0xC0 + 15));
      }
}
...
char change_password (unsigned char key,unsigned char reset_flag)
{
    ...
    if(pos == 8)
    {
        if(strncmp(pwd,&pwd[4],4) == 0)
        {
            ...
            clcd_write(0x0C, 0);
           _delay((unsigned long)((100)*(20000000/4000000.0)));
           clcd_print("Password change",(0x80 + 1));
           clcd_print(" Successfully ",(0xC0 + 1));
           _delay((unsigned long)((3000)*(20000000/4000.0)));
           pos = 0;
           return 0x0C;
...
void change_rtc_time(unsigned char key, unsigned char reset_flag)
{
    ...
    char dummy;
    char hour = (time[0] & 0x0F ) * 10 + (time[1] & 0x0F),
          min = (time[2] & 0x0F ) * 10 + (time[3] & 0x0F),
          sec = (time[4] & 0x0F ) * 10 + (time[5] & 0x0F);


    if(key == 0x37 && time_count < 3)
    {
        time_count++;
    }
    if(key == 0x2F && time_count > 0)
    {
        time_count--;
    }
  ...
  break;
    ...
    clcd_putch(time[0], (0xC0 + 3));
    clcd_putch(time[1], (0xC0 + 4));
    clcd_putch(':', (0xC0 + 5));
    clcd_putch(time[2], (0xC0 + 6));
    clcd_putch(time[3], (0xC0 + 7));
    clcd_putch(':', (0xC0 + 8));
    clcd_putch(time[4], (0xC0 + 9));
    clcd_putch(time[5], (0xC0 + 10));
}
...
void ds1307_off(void)
{
    ...
    dummy_off = read_ds1307(0x00);
    dummy_off = dummy_off | 0x80;
    write_ds1307(0x00, dummy_off);
}
...
void ds1307_on(void)
{
    ...
    dummy_on = read_ds1307(0x00);
    dummy_on = dummy_on & 0x7F;
    write_ds1307(0x00, dummy_on);
}
