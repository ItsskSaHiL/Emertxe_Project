...
void main(void)
{
    ...
    unsigned char key,key_level,gr = 0;
    char event[] = {"ON"};
    unsigned char speed = 0,count = 0,pwd_flag = 0,wait = 0;
    char login_menu_pos,sw4_flag = 0;
    char d_log[11];
    d_log[10] = '\0';
    unsigned char add_d;
    char ee_pos_d;
    ...
    while (1)
    {
        ...
        if(key == 0x3E && control_flag == 0x01)
        {
            strcpy(event,"C ");
            log_car_event(event,speed);
            count++;
        }
        if(key == 0x3D && (gr < 6) && control_flag == 0x01 )
        {
                     ...
                     if(ch != '\0')
                     {
                        ...
                        puts("SL No     TIME     EVENT     SPEED\n");
                         if(event_count == -1)
                          ...
                          {
                              for(ee_pos_d = 0;ee_pos_d < event_count;ee_pos_d++)
                              {
                                  for(int i = 0; i < 10;i++)
                                  {
                                    add_d = ee_pos_d * 10 + 5;
                                    d_log[i] = read_24c04(add_d + i);
                                  }
                              ...
                              clear_screen();
                              clcd_print("Logs Download ",(0x80 + 2));
        ...
        else if(((control_flag == 0x05) || (control_flag == 0x08)) && key == 0x12)
        {

             if(control_flag == 0x08)
             {
                 clear_screen();
                 clcd_print("time change",(0x80 + 2));
                 clcd_print("successfully",(0xC0 + 0));
                 _delay((unsigned long)((3000)*(20000000/4000.0)));
                 clear_screen();
                 ds1307_on();
        ...
        else if ( key == 0x11 && control_flag == 0x03 )
        {
                control_flag = 0x01;
                clear_screen();
        }
        ...
        switch(control_flag)
        {
            ...
            case 0x02:
                    ...
                    break;
            case 0x03:
                  login_menu_pos = login_menu(key,reset_flag);
                  break;
            case 0x05:
            ...
            case 0x06 :
                ...
                break;
            case 0x08:
                clcd_print("Change the Time",(0x80 + 1));
                change_rtc_time(key,reset_flag) ;
                break;
            case 0x07:
                if(change_password (key, reset_flag) == 0x0C)
                {
                     clear_screen();
                     clcd_write(0x0C, 0);
                     _delay((unsigned long)((100)*(20000000/4000000.0)));
